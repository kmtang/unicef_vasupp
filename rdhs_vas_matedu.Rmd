...
```{r}
c1 <- data %>% dplyr::select(caseid, v001, v005, b8_01, b19_01, h33_1, v106)
c2 <- data %>% dplyr::select(caseid, v001, v005, b8_02, b19_02, h33_2, v106)
c3 <- data %>% dplyr::select(caseid, v001, v005, b8_03, b19_03, h33_3, v106)
c4 <- data %>% dplyr::select(caseid, v001, v005, b8_04, b19_04, h33_4, v106)
c5 <- data %>% dplyr::select(caseid, v001, v005, b8_05, b19_05, h33_5, v106)
c6 <- data %>% dplyr::select(caseid, v001, v005, b8_06, b19_06, h33_6, v106)

names(c1) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
names(c2) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
names(c3) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
names(c4) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
names(c5) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
names(c6) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "matedu")
...
```{r}
c1 <- data %>% dplyr::select(caseid, v001, v005, b8_01, h33_1, v106)
c2 <- data %>% dplyr::select(caseid, v001, v005, b8_02, h33_2, v106)
c3 <- data %>% dplyr::select(caseid, v001, v005, b8_03, h33_3, v106)
c4 <- data %>% dplyr::select(caseid, v001, v005, b8_04, h33_4, v106)
c5 <- data %>% dplyr::select(caseid, v001, v005, b8_05, h33_5, v106)
c6 <- data %>% dplyr::select(caseid, v001, v005, b8_06, h33_6, v106)

names(c1) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
names(c2) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
names(c3) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
names(c4) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
names(c5) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
names(c6) <- c('caseid',"cluster", "weight",'age', 'vas', "matedu")
...
data <- data %>% mutate(matedu_cat = case_when(matedu=="no education" ~ "None",
...
SZ<- w_data %>%
  filter(!is.na(matedu_cat)) %>% 
  srvyr::group_by(matedu_cat) %>%
  srvyr::summarise(
    vasupp = (survey_mean(vasupp == 1, proportion = TRUE, vartype = "ci",na.rm = T)) * 100)

SZ$country <- "ESwatini"

vas_cov <- rbind(BJ,BF,CM,ET,MD,MW,ML,NP,NI,PK,ZA,AF,AO,BO,BU,KH,TD,CG,CD,CI,GA,BD,KM,SZ,GM,GH,GN,HT,KE,LS,LB,MZ,MM,NM,NG,PG,PH,RW,ST,SN,SL,TJ,TZ,TL,TG,UG,ZM,ZW,SZ)
```

Plot
```{r}
ggplot(vas_cov) + 
  geom_pointrange(aes(x = reorder(country, order), y = vasupp, ymin = vasupp_low, ymax = vasupp_upp, color = matedu_cat))+ 
  theme_bw() +
  coord_flip(ylim = c(0, 100)) +
  labs(y = "Percentage of children consuming VAS in the prior 6 months",
       x = "Country",
       caption = "*Yemen did not collect dietary recall data in DHS individual recode") +
...
order <- vas_cov %>% filter(matedu_cat=="Secondary+") %>% arrange(vasupp) %>% select(country) %>% dplyr::mutate(order = row_number())
...
vas_cov <- read.csv("/Users/kevintang/Library/Mobile Documents/com~apple~CloudDocs/Documents/r_projects/unicef_vasupp/data/vas_matedu.csv")
```
