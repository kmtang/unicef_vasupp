...
c1 <- data %>% dplyr::select(caseid, v001, v005, b8_01, h33_1, h4_1)
c2 <- data %>% dplyr::select(caseid, v001, v005, b8_02, h33_2, h4_2)
c3 <- data %>% dplyr::select(caseid, v001, v005, b8_03, h33_3, h4_3)
c4 <- data %>% dplyr::select(caseid, v001, v005, b8_04, h33_4, h4_4)
c5 <- data %>% dplyr::select(caseid, v001, v005, b8_05, h33_5, h4_5)
c6 <- data %>% dplyr::select(caseid, v001, v005, b8_06, h33_6, h4_6)

names(c1) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
names(c2) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
names(c3) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
names(c4) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
names(c5) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
names(c6) <- c('caseid',"cluster", "weight",'age', 'vas', "polio")
...
vas_cov <- rbind(AO,BD,BJ,BU,CM,ET,GM,GN,HT,LB,MW,ML,NP,NG,PK,PG,PH,SN,SL,ZA,TJ,TZ,UG,ZM,ZW,
                 AF,BO,BF,KH,TD,KM,CG,CD,CI,SZ,GA,GH,KE,LS,MD,MZ,MM,NM,NI,RW,ST,TG,YE)

ggplot(vas_cov) + 
  geom_pointrange(aes(x = reorder(country, order), y = vasupp, ymin = vasupp_low, ymax = vasupp_upp, color = polio_cat))+ 
  theme_bw() +
...
order <- vas_cov %>% filter(polio_cat=="yes") %>% arrange(vasupp) %>% select(country) %>% dplyr::mutate(order = row_number())
vas_cov<- merge(x=vas_cov, y=order, by.x='country', by.y='country', fill=-9999, all.x = TRUE)

write.csv(vas_cov, "/Users/kevintang/Library/Mobile Documents/com~apple~CloudDocs/Documents/r_projects/unicef_vasupp/data/vas_polio.csv", row.names=FALSE)

vas_cov <- read.csv("/Users/kevintang/Library/Mobile Documents/com~apple~CloudDocs/Documents/r_projects/unicef_vasupp/data/vas_polio.csv")
```
...
c1 <- data %>% dplyr::select(caseid, v001, v005, b8_01, b19_01, h33_1, h4_1)
c2 <- data %>% dplyr::select(caseid, v001, v005, b8_02, b19_02, h33_2, h4_2)
c3 <- data %>% dplyr::select(caseid, v001, v005, b8_03, b19_03, h33_3, h4_3)
c4 <- data %>% dplyr::select(caseid, v001, v005, b8_04, b19_04, h33_4, h4_4)
c5 <- data %>% dplyr::select(caseid, v001, v005, b8_05, b19_05, h33_5, h4_5)
c6 <- data %>% dplyr::select(caseid, v001, v005, b8_06, b19_06, h33_6, h4_6)

names(c1) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
names(c2) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
names(c3) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
names(c4) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
names(c5) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
names(c6) <- c('caseid', "cluster", "weight", 'age', 'age_m', 'vas', "polio")
