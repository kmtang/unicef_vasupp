...
for(survid in ird$SurveyId){
  ...
  dat <- dat[grep("caseid|^h33|^b19|v224", names(dat))]
  ir[[survid]] <- dat
}
...
#Vitamin A
data <- ir$MW2015DHS
subset(data, h34_1 == "yes")
nrow(data)
ldply(ir, function(x){ nrow(subset(x, h34_1 == "yes"))/nrow(x)})

data <- ir$MW2015DHS
data <- subset(data, v102 == "urban")
nrow(data)

fn.rowcount <- function(x){ x <- subset(x, v102 == "urban")
nrow(x)}

l.res <- lapply(ir, fn.rowcount)
class(l.res)

df.res <- plyr::ldply(ir, fn.rowcount, .id = "mysurvey")
df.res
